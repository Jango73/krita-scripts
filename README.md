...
## Quick usage
1) Open Krita, go to Tools → Scripts → ComfyUI Image Enhance.
2) In the dialog, set the ComfyUI server URL (see the manual).
3) Optionally select regions in your image; they will be enhanced separately.
4) Click “Go” (global + regions) or “Regions” (regions only). Results appear as new layers in Krita.

## Things to know
- During the enhancing process, the Krita UI responds less frequently to user input, this is normal.
