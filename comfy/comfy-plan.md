...
- Configuration storage (server, workflows, prompts, parameter lists) in a JSON file (Krita user directory).

2) User interface
- Text fields: server address, global workflow name, region workflow name.
- Prompts: 1 "global prompt" field, 4 "region prompt 1..4" fields; keep current value (optional: in-session history).
- Variable parameters: two tables (global, region) with columns "Parameter name" and "Value"; Add/Remove buttons; simple validation (non-empty).
- Log area (QTextEdit read-only, monospace) + "Clear" button.
- Buttons: "Go" (global + regions), "Regions" (regions only), optional "Test connection".

3) Data preparation
- On "Image enhance" click: verify a document is open and non-empty.
- Get all rectangular selections from the active view; sort by ascending X (left to right) to order regions.
- Export the full image to a temporary JPG for the global workflow.
- For each rectangle, extract the corresponding portion to a temporary JPG (keep width/height and original position).
...
- Once ready, locate the generated file in the ComfyUI output location, load it into Krita as a new layer named "Global enhance" with the configured opacity.

5) ComfyUI call: region loop
- Choose the prompt per region: index 0 -> prompt 1, index 1 -> prompt 2, index 2 -> prompt 3, index >= 3 -> prompt 4.
- For each region:
  - Load the region workflow, inject the cropped image into "Load Image" (name or ID) and the selected prompt into the text node named "Prompt" (or by ID if needed).
  - Apply region parameters (same replacement logic as global).
- Send the workflow to ComfyUI, wait for the output, retrieve the generated file.
- Insert the image as a layer "Region enhance #n" positioned at the same offset as the source rectangle.
...
- Set the layer opacity to the configured value.
...
- Write every step to the log area: start/end global, start/end region n, parameter replacements, ComfyUI success/errors, output paths.
...
- Save preferences (server, workflow names, prompts, parameters) when closing the dialog or after a successful run.
- Default path in Krita user directory (e.g. `~/.krita/comfy_config.json`).

- 1 to 3 regions: matching prompts used; layers inserted and masked.
- >=4 regions: region prompt 4 reused; verify the 10% fade on each region.
